...
> テンプレの核は `docs/TEMPLATE_DESIGN.md`。  
> ここでは **環境準備と初回起動**に絞ります。
...
- [ ] `scripts/run.ps1` / `scripts/run.sh` で **最小確認**ができる（`out/repo_dump.txt` 生成）
- [ ] `docs/ai/` を読ませれば、端末が変わっても“昨日の続き”が再開できる
- [ ] `docs/TEMPLATE_DESIGN.md` の運用原則を満たしている
...
## 2. テンプレートrepo方式（推奨フロー）
この2つを分けて考えると迷いません。

### 2.1 初回だけ：テンプレートrepoを用意する（1回だけ）
...
## 8. 最小確認（必須：3分で動作確認）
`scripts/run.ps1` / `scripts/run.sh` を実行して、  
`out/repo_dump.txt` が生成されればOK。

---

## 9. GitHub側の推奨設定（壊さないため）
